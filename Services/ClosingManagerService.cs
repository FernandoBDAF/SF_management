...
{
    public ClosingManagerService(DataContext context, IHttpContextAccessor httpContextAccessor) : base(context,
        httpContextAccessor)
    {
    }
    ...
    {
        var manager = await context.Managers.Include(x => x.Wallets).ThenInclude(x => x.Nicknames)
            .FirstOrDefaultAsync(x => x.Id == obj.ManagerId);
    ...
    public async Task<ClosingManager> Done(Guid closingManagerId)
    {
        ...
        if (closingManager == null) throw new AppException("Not found closing manager.");

        if (closingManager.DoneAt.HasValue) throw new AppException("Closing manager closed.");

        closingManager.CalculatedAt = DateTime.Now;
        closingManager.RakeBruto =
            ClosingManager.CalcRake(closingManager.ClosingNicknames, closingManager.ClosingWallets);
        closingManager.TotalBalance = closingManager.ClosingNicknames.Sum(x => x.Balance);

        if (closingManager.RakeBruto != decimal.Zero)
            closingManager.Manager.InternalTransactions.Add(ClosingManager.CreateRakeInternalTransaction(
                closingManager.ManagerId, closingManager.RakeBruto, closingManager.Manager?.Name, closingManager.End,
                closingManagerId));

        var nicknameDiscounts = ClosingManager.CreateRakeNicknameReleases(closingManager.ManagerId,
            closingManager.ClosingNicknames, closingManager.Manager.Name, closingManager.End, closingManager.Id);
