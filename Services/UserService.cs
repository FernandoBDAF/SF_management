...
public class UserService
{
    ...
    {
        return _mapper.Map<List<UserResponse>>(await _userManager.Users.ToListAsync());
    }
    ...
    public async Task<JwtSecurityToken> CreateJwtToken(ApplicationUser user)
    {
        ...
        return new JwtSecurityToken(_jwt.Issuer, _jwt.Audience, claims,
            expires: DateTime.UtcNow.AddMinutes(_jwt.DurationInMinutes), signingCredentials: signingCredentials);
    }
    ...
    public async Task<string> AddRoleAsync(AddRoleRequest model)
    {
        ...
        if (await _userManager.CheckPasswordAsync(user, model.Password))
        {
            var roleExists = Enum.GetValues(typeof(Authorization.Roles)).Cast<Authorization.Roles>()
                .Any(x => x.ToName<Authorization.Roles>().ToLower() == model.Role.ToLower());
    ...
    public async Task<AuthenticationModel> RefreshToken(TokenRequest model)
    {
        ...
        if (model is null) throw new AppException("Invalid client request");

        var accessToken = model.AccessToken;
        var refreshToken = model.RefreshToken;

        var principal = GetPrincipalFromExpiredToken(accessToken);
        if (principal == null) throw new AppException("Invalid access token/refresh token");
